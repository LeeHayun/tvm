...
bool SparseConv2DRel(const Array<Type>& types, int num_inputs, const Attrs& attrs,
  ...
  if (param->block_size.defined() && param->num_nnz_blocks.defined() && param->channels.defined()) {
    ...
    // assign result to reporter
    reporter->Assign(types[1], TensorType(wdata_shape, weight_dtype));
    reporter->Assign(types[2], TensorType(windices_shape, DataType::Int(32)));
    reporter->Assign(types[3], TensorType(windptr_shape, DataType::Int(32)));
  } else {
    // use weight to infer the conv shape.
