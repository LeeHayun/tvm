...
/*! \brief Attributes used in sparse convolution operators */
struct SparseConv2DAttrs : public tvm::AttrsNode<SparseConv2DAttrs> {
  ...
  TVM_DECLARE_ATTRS(SparseConv2DAttrs, "relay.attrs.SparseConv2DAttrs") {
    TVM_ATTR_FIELD(block_size).set_default(NullValue<Array<IndexExpr> >())
        .describe("Specifies the block size to use for sparse convolution.");
    TVM_ATTR_FIELD(num_nnz_blocks).set_default(NullValue<IndexExpr>())
        .describe("Specifies the number of non-zero blocks (BSR format)");
    TVM_ATTR_FIELD(strides).set_default(Array<IndexExpr>({1, 1}))
        .describe("Specifies the strides of the convolution.");
    TVM_ATTR_FIELD(padding).set_default(Array<IndexExpr>({0, 0}))
        .describe("If padding is non-zero, then the input is implicitly zero-padded"
